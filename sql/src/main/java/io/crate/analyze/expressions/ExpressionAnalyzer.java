...
public class ExpressionAnalyzer {
    ...
    private FunctionInfo getFunctionInfo(String name, List<DataType> arguments) {
        return getFunctionInfo(Schemas.DEFAULT_SCHEMA_NAME, name, arguments);
    }
    ...
    protected Symbol convertFunctionCall(FunctionCall node, ExpressionAnalysisContext context) {
        ...
        if (fName.size() == 1) {
            fName = ImmutableList.of(Schemas.DEFAULT_SCHEMA_NAME, fName.get(0));
        }
        ...
        if (node.isDistinct()) {
            ...
            Symbol innerFunction = context.allocateFunction(
                getFunctionInfo(CollectSetAggregation.NAME, argumentTypes),
                arguments
            );
    ...
    private class InnerExpressionAnalyzer extends AstVisitor<Symbol, ExpressionAnalysisContext> {
        ...
        Symbol resolveSubscriptSymbol(SubscriptContext subscriptContext, ExpressionAnalysisContext context) {
            ...
            if (index != null) {
                ...
                return context.allocateFunction(
                    getFunctionInfo(SubscriptFunction.NAME, ImmutableList.of(subscriptSymbol.valueType(), indexSymbol.valueType())),
        ...
        protected Symbol visitNotExpression(NotExpression node, ExpressionAnalysisContext context) {
            ...
            return context.allocateFunction(
                getFunctionInfo(NotPredicate.NAME, Arrays.asList(argument.valueType())),
        ...
        protected Symbol visitArithmeticExpression(ArithmeticExpression node, ExpressionAnalysisContext context) {
            ...
            return context.allocateFunction(
                getFunctionInfo(node.getType().name().toLowerCase(Locale.ENGLISH), Arrays.asList(left.valueType(), right.valueType())),
        ...
        public Symbol visitArrayLiteral(ArrayLiteral node, ExpressionAnalysisContext context) {
            ...
            if (values.isEmpty()) {
                ...
                return context.allocateFunction(getFunctionInfo(ArrayFunction.NAME, Symbols.extractTypes(arguments)), arguments);
            }
        }
        ...
        public Symbol visitObjectLiteral(ObjectLiteral node, ExpressionAnalysisContext context) {
            ...
            return context.allocateFunction(getFunctionInfo(MapFunction.NAME, Symbols.extractTypes(arguments)), arguments);
        }
