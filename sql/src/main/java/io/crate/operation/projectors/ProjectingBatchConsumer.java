...
/**
 * Consumer implementation which wraps the BatchIterator received on {@link #accept(BatchIterator, Throwable)}, applying
 * any projections.
 *
 * {@link #consumer} will receive the projected BatchIterator.
 */
    ...
    public void accept(BatchIterator iterator, @Nullable Throwable failure) {
        if (failure == null) {
            for (Projection projection : projections) {
                iterator = projectorFactory.projectIterator(iterator, projection, ramAccountingContext, jobId);
            }
