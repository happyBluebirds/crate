...
public class ShardCollectSource extends AbstractComponent implements CollectSource {
    ...
    public Collection<CrateCollector> getCollectors(CollectPhase phase,
        ...
        BatchConsumer firstNodeConsumer = ProjectingBatchConsumer.create(
            lastConsumer,
        ...
        if (normalizedPhase.maxRowGranularity() == RowGranularity.SHARD) {
            ...
            return Collections.singletonList(
                getShardsCollector(collectPhase, normalizedPhase, localNodeId, firstNodeConsumer));
        }
        ...
        switch (builders.size()) {
            case 0:
                return Collections.singletonList(RowsCollector.empty(firstNodeConsumer));
            case 1:
                CrateCollector.Builder collectorBuilder = builders.iterator().next();
                return Collections.singletonList(
                    collectorBuilder.build(collectorBuilder.applyProjections(firstNodeConsumer)));
            default:
                if (hasShardProjections) {
                    ...
                    return Collections.singletonList(
                        CompositeCollector.syncCompositeCollector(builders, firstNodeConsumer));
                }
        }
    }
